...
这等价于把观测图像分解为光照与反射两个因子的乘积，增强只针对光照项下手。从观测图像中解出反射分量，$R(x, y)$ 的计算如下：

$$R(x, y) = \frac{S(x, y)}{L(x, y)}$$

其中 $S(x, y)$ 是人眼捕获的视觉图像，$L(x, y)$ 是入射图像。这等价于在分解式两边同除光照分量，增强的难点归结为估计 $L(x, y)$。对上式取对数可得：

$$\ln R(x, y) = \ln S(x, y) - \ln L(x, y)$$

其中各项含义同上。直观上，取对数把乘性的光照影响变为减性项，照度估计的误差以减法形式进入结果。由于入射光线强度在被照表面变化相对较慢，$L(x, y)$ 可用图像低频分量表示，即以像素点与周围区域的加权平均 $F(x, y)$ 估计照度并去除，入射图像估计为：

$$L(x, y) = F(x, y) \cdot S(x, y)$$

其中 $F(x, y)$ 为中心环绕函数，$F(x, y) \cdot S(x, y)$ 表示以该函数对图像加权平均来估计照度。把该估计代回对数式，对式子取对数后得到：

$$\ln R(x, y) = \ln S(x, y) - \ln[F(x, y) \cdot S(x, y)]$$

其中 $F(x, y)$ 为中心环绕函数。直观上，把乘性的光照分量在对数域中减掉，剩下的就是与光照无关的物体反射属性；将 $\ln R(x, y)$ 变换回实数域即得到增强图像。增强前后的效果如下图所示。

![原始图像及 Retinex 算法增强效果图像](../assets/paper-imgs/TPH-YOLO/fig2.png)

图中 (a) 为原始图像、(b) 为 Retinex 增强后的图像，增强后麦穗与背景的对比更清晰，作为检测网络的输入。
...
图中 (a) 为原图、(b) 与 (c) 分别为 YOLOv5 与本文模型的检测结果，方框表示预测框。麦穗重叠问题上的局部放大对比如下图所示，箭头标出漏检或误检位置。
