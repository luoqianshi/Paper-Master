...
图中给出三个生成器的内部结构，它们都以初始融合特征 $Z_l$ 为输入，分别输出逐像素的低通核、重采样偏移与高通核，最终融合的平滑、重采样与锐化三条路径由此并行展开。
...
图中 (a) 是生成 3×3 空间可变低通核的 9 个学习卷积核，亮度代表权重大小，(b) 为对应的傅里叶变换核，(c) 的平均频谱显示高频段功率更高，可见滤波器的预测依赖输入特征中的高频成分。
...
![特征相似性分析的定量结果](../assets/paper-imgs/FreqFusion/TABLEI.png)

表中列出标准特征融合、完整 FreqFusion 与各去掉一个生成器的消融变体在整体与边界两区域上的三组相似度指标，完整 FreqFusion 行在 6 列指标上全部取得最优值，可见三个生成器的增益可以在同一标尺下逐项归因。
...
图中灰色 3×3 网格给出像素 $(i,j)$ 与 8 邻域的余弦相似度，亮度越高相似度越大，偏移生成器据此把采样点引向类内相似度更高的邻居，边界与不一致区域的模糊因此被压缩。
...
图中以对数坐标给出各频段的功率对比，可以看到 AHPF 生成器抬升了高频功率，特征的细节水平与边界清晰度随之提升，与上文看到的边界增强互为印证。
...
从表中可以看出，在同一分割模型下更换融合与上采样模块的各方法里 FreqFusion 排名第一，以 1.2 mIoU 的优势领先第二名 Dysample-S+，可见两个缺陷的缓解直接转化为分割精度的提升。
...
从表中可以看出，三种结构各自的 FreqFusion 行相对基线行全部抬升 1.0 mIoU 以上，而参数量与 FLOPs 的增量很小，表明该融合模块对主干与解码结构并不挑剔。
...
从表中可以看出，三个数据集上基线到 FreqFusion 的抬升幅度都在 1.0 mIoU 以上，表明增益能够跨数据集复现。
...
从表中可以看出，FreqFusion 的 39.4 AP 是表内最高值，ResNet-50 版本与更深的 ResNet-101 最近邻基线持平，可见检测精度的收益同样来自融合质量而非骨干加深。
...
从表中可以看出，box AP 与 mask AP 两列上 FreqFusion 在两种骨干下都排在首位，表明检测定位与掩码分割两个目标可以同时受益。
...
从表中可以看出，ResNet-50 与 ResNet-101 两个版本的 PQ 均高于表内其余融合方法，R101 版本达到 44.0 PQ，可见全景分割同样从融合改进中稳定获益。
...
从表中可以看出，三个生成器逐个叠加时每一步都继续涨点，三者齐上达到最高的 43.5 mIoU，表明低通、重采样与高通三条路径的收益可以逐项归因。
...
从表中可以看出，低通核从 3 增到 5 后继续增到 7 保持稳定，高通核从 3 增到 5 则掉到 42.4，可见低通核可以安全增大而高通核不能。

偏移分组数的作用则是把重采样做得更细：4 组时取得最优的 43.5 mIoU，继续增大不再受益甚至略有回落。

![偏移分组数的消融](../assets/paper-imgs/FreqFusion/TABLEXIV.png)

从表中可以看出，偏移分组数取 4 时 mIoU 达到最优的 43.5，继续增大反而略有回落，可见分组粒度存在饱和点。
...
图中红框为放大区域，标准融合下目标内部的特征斑驳发碎、边界发虚，换用 FreqFusion 后内部一致性与边界锐度同时改善，可见前文相似度指标的提升对应着肉眼可辨的特征变化。
...
图中第一行为 SegNeXt 基线、第二行为换用 FreqFusion 的结果，第二行中同一目标的预测类别更统一、边界更连续，可见特征层面的改善能直接传导到分割输出。ADE20K 验证集上的分割对比如下图所示，图中三行依次为 SegFormer 基线、换用 FreqFusion 的结果与真值标注。

![ADE20K 验证集上的分割可视化对比](../assets/paper-imgs/FreqFusion/fig14.png)

虚线框标出的对比区域内，基线的碎块状错分在换用 FreqFusion 后被修复，预测与第三行的真值更加接近，表明跨数据集的一致性改善与 Cityscapes 上的表现同步。
