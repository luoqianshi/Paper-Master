...
该模块回答「核生成应当使用哪些特征」，结论是两类特征缺一不可。本文在 SUN RGBD 语义分割与 Fashion-MNIST 图像重建上构建三个基线：decoder-only（CARAFE 的标准实现）、encoder-only、encoder-decoder 拼接，仅改变核生成的特征来源。结果显示 decoder-only 在分割上取得 mIoU 37.00 与 bIoU 25.61，但重建 PSNR 仅 24.35；encoder-only 相反，重建 PSNR 达 32.25 而分割 mIoU 降到 36.71；encoder-decoder 在两个任务上同时取到最优，mIoU 37.59、bIoU 28.80、PSNR 33.83。三个基线的分割掩码与重建结果可视化对比如下图所示，原文 Fig 5 解释了原因。

![三种特征来源的分割掩码与重建结果对比（原文 Fig 5）](../assets/paper-imgs/FADE/fig5.png)

图中 decoder-only 的掩码区域连贯但重建低保真，encoder-only 的边界清晰但区域块状，二者结合后同时继承两侧优点，红框标出的台灯与衣物条纹即为直观例子。可见高分辨率编码器特征负责细节预测、低分辨率解码器特征负责区域语义保持，这一结论直接支撑 FADE 的联合核生成设计，并为下一节半移位卷积模块提供了需要融合的两路输入。

### 半移位卷积：原理

半移位卷积用于解决引入两类特征后的分辨率失配问题，把特征插值、拼接、通道压缩、核生成四步统一为一个算子。朴素实现先把解码器特征插值到编码器分辨率再拼接卷积，但 ×2 最近邻插值配 3×3 卷积会使有效感受野降到 50% 以下：插值前窗口内有 9 个有效点，插值后只剩 4 个；且控制高分辨率 2×2 邻域核方差的窗口对低分辨率解码器特征而言是盲的。这一朴素实现的结构如下图所示。

![联合核生成的朴素实现结构（原文 Fig 3）](../assets/paper-imgs/FADE/fig3.png)

图中解码器特征先经 NN 插值对齐到 2H×2W 再与编码器特征拼接，随后经通道压缩器、3×3 卷积与 softmax 生成上采样核，分辨率对齐完全依赖显式插值。半移位卷积让编码器特征上的 4 个卷积窗口对应解码器特征上的同一个窗口，如下图所示。
...
门控机制用于在上采样之后从编码器特征中继续补偿细节。在 Weizmann Horse 二值分割数据集上，当各算子的区域预测都接近完美时细节差距被放大，各算子的 mIoU 对比如下表所示。

![Weizmann Horse 上各算子的二值分割 mIoU 对比（原文 Table 2）](../assets/paper-imgs/FADE/table2.png)

以 SegNet 为基线，Unpooling 与 IndexNet 取得 mIoU 93.42 与 93.00，NN 与 CARAFE 只有 89.15 与 89.29，而给 NN 和 CARAFE 加上门控后分别升到 95.26 与 95.25，可见门控的细节补偿对两种核生成方式都有效。原文 Fig 8 的梯度图与门控图如下图所示，其中中间解码器层的梯度图显示训练中期大部分响应集中在边界附近，说明解码器特征自身已编码了对细节的需求分布。

![马匹分割的梯度图与门控图（原文 Fig 8）](../assets/paper-imgs/FADE/fig8.png)

图中门控图的高响应集中在马匹轮廓等需要补偿细节的位置、区域内部保持低响应，与梯度图的分布相互对应。据此本文用 1×1 卷积、NN 插值与 sigmoid 生成门控图，调制编码器特征与上采样特征，计算为：
...
FADE 配合 SegFormer-B1 取得 44.41 mIoU 与 32.65 bIoU，相对 Bilinear 的 41.68 与 27.80 提升 +2.73 与 +4.85；抠图 SAD 从 37.31 降到 31.10，相对改进 16–32%，且 FADE-Lite 以更少参数仍超过强基线 A2U，表明任务无关性不依赖参数量。换用更强骨干时 FADE 的 mIoU 增益逐渐饱和（B3 上仅 +0.01）但 bIoU 稳定提升 1 个点以上，说明强骨干的特征质量主要修复了区域误分类，边界误差仍要靠上采样弥补。换用 UPerNet 架构后的结果如下表所示。

![UPerNet 架构下的语义分割对比结果（原文 Table 6）](../assets/paper-imgs/FADE/table6.png)

FADE 在 ResNet-50 与 ResNet-101 上分别取得 42.18 与 44.27 mIoU，都比 Bilinear 提升约 1 个 mIoU 并超过 CARAFE，可见增益并不依赖特定架构。COCO 检测中带门控的 FADE 只有 37.8 AP，置 $G = 1$ 后升到 38.5、接近 CARAFE 的 38.6，ResNet-101 上达到 40.0 AP；实例分割中 FADE (G=1) 与 CARAFE 持平。NYU Depth V2 上的结果见原文 Table 9，截图如下。
...
检测器偏好细节丰富的特征，这解释了检测任务中 $G = 1$ 更优的现象：门控压制了本可用于定位的编码器细节。Mask R-CNN-R50 中各上采样器输出的特征图如下图所示。

![Mask R-CNN-R50 上采样器输出的特征图对比（原文 Fig 12）](../assets/paper-imgs/FADE/fig12.png)

图中上半部分对比 CARAFE、IndexNet、A2U、SAPA、FADE 五种上采样算子输出的特征图，下半部分对比 Nearest、FA2M、FAM、GD-FAM 四种对齐方式，不同组合的特征图清晰度差异明显，可见特征图质量可以作为性能好坏的暗示。跨任务的定性效果汇总如下图所示。

![五类密集预测任务上的定性效果对比（原文 Fig 10）](../assets/paper-imgs/FADE/fig10.png)

原文 Fig 10 的跨任务定性对比进一步显示，所有对比算子中只有 FADE 在分割、抠图、检测、实例分割与深度估计上同时给出视觉满意的结果，与定量指标互相印证。
