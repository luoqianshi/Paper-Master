...
本文方法是一条「切块—超分—增强—增广—训练—检测—拼图」的流水线：四个试验小区的 132 幅无人机茶图像被切为 640×640 图像块，小区①②③ 中含 TLB 的 300 个图像块作为训练图像，小区④ 中含 TLB 的 55 幅图像作为测试图像。数据采集区域与茶园位置如下图所示。

![数据采集区域与茶园位置](../assets/paper-imgs/DDMA-YOLO/fig1.png)

图中上部为试验茶园的无人机航拍影像，下部为采集区域的地理位置，虚线把航拍区域连到地图上的对应地点，四个试验小区的图像即采集自该区域。切块后的图像样例如下图所示。
...
其中 $F(x, y) * S(x, y)$ 是对照射分量的高斯核卷积近似。直观上，取对数把乘性光照变为加性项再减去，剩下的就是与光照无关的物体反射属性。上述计算的总体流程如下图所示。

![Retinex 算法总体流程](../assets/paper-imgs/DDMA-YOLO/fig7.png)

图中给出对数域的处理路径：观测图像的对数 $\log S(x, y)$ 与照射分量近似的对数 $\log[F(x, y) * S(x, y)]$ 相减，得到反射分量的对数 $\log R(x, y)$，即上式的逐步执行过程。处理前后的图像对比如下图所示。

![Retinex 处理前后的图像](../assets/paper-imgs/DDMA-YOLO/fig8.png)

图中 (a) 为原始图像、(b) 为 Retinex 处理后的图像，增强后的图像对比度更明显、光照不均的影响被削弱，作为检测网络的输入。
...
图中给出病叶像素尺寸的分布统计，小尺寸病叶的数量占了多数，与上文的小目标判断一致。增广后的训练集送入 DDMA-YOLO 训练，测试集不参与增广。
...
DDMA 模块用于在颈部抑制冗余背景、聚焦密集分布的病叶。作为对照的 CBAM 把通道注意力与空间注意力串联起来，其结构如下图所示。

![CBAM 模块结构](../assets/paper-imgs/DDMA-YOLO/fig13.png)

图中特征先经通道注意力加权、再与原特征相乘送入空间注意力，两级编码相互独立，只能捕获特征的局部相关性，缺少解决密集分布问题所需的非局部信息。本文把局部与非局部注意力并联起来，DDMA 的结构如下图所示。
...
其中 TP、FP、FN 分别为真正例、假正例与假负例数量；P 与 R 为精确率与召回率；AP 为单类别的平均精度，AP@0.5:0.95 取 AP@0.5 至 AP@0.95 十个值的平均。模型训练曲线如下图所示。

![模型训练曲线](../assets/paper-imgs/DDMA-YOLO/fig15.png)

图中同时画出损失与精确率、召回率、AP@0.5 的训练曲线，损失曲线在 200 轮后趋于稳定，可见模型基本收敛。
...
图中并排给出 bicubic、SRCNN 与 RCAN 三种方法对同一批茶图像的重建效果。三种重建方法的定量指标如下表所示。
