...
从纯编码器出发的 80.10% 开始，级联结构带来 +0.98%，LGAG 与 MSCAM 单独接入分别到 81.92% 与 82.86%，两者合用达到 83.63%，总代价仅 0.381G FLOPs 与 1.91M 参数，可见 MSCAM 的贡献大于 LGAG，二者互补。MSDC 核尺寸组合的消融结果如下表所示。

![MSDC 多尺度核组合消融](../assets/paper-imgs/EMCAD/table5.png)

单核从 1×1 到 3×3 递增（Synapse 82.43%→82.79%），[1, 3] 优于任一单核，[3, 3] 反而下降到 82.81%，加入 5×5 后的 [1, 3, 5] 在 Synapse（83.63%）与 ClinicDB（95.21%）上均最优，继续加 7×7、9×9 大核则双双掉点，说明多尺度的收益来自尺度多样性而非核尺寸本身。与基线解码器 CASCADE 的复杂度与精度直接对比如下表所示。

![与基线解码器 CASCADE 的复杂度对比](../assets/paper-imgs/EMCAD/table6.png)

PVTv2-B2 下 EMCAD 解码器以 0.381G FLOPs 与 1.91M 参数取得 83.63% DICE，比 CASCADE（1.93G、9.27M、82.78%）少 80.3% FLOPs 与 79.4% 参数且高 0.85% DICE；PVTv2-B0 下以少 74.9% FLOPs、78.1% 参数取得 +1.43%，可见深度卷积替代标准卷积的效率优势在两个规模上都成立。MSDC 中并行与串行两种深度卷积排布的对比如下表所示。

![并行与串行深度卷积排布对比](../assets/paper-imgs/EMCAD/table7.png)

并行排布在四个设置中全部略高于串行（0.03% 至 0.15%），且串行的多次运行标准差更高，可见并行排布在精度与稳定性上都更好，默认取并行。LGAG 与原注意力门 AG 的开销与精度对比如下表所示。

![LGAG 与 AG 的开销对比](../assets/paper-imgs/EMCAD/table8.png)

B2 下三个门的参数从 124.68K 降到 11.01K、FLOPs 从 61.68M 降到 10.47M，DICE 从 83.51% 升到 83.63%；B0 下参数从 31.62K 降到 5.51K 且 DICE 提升到 81.97%，表明分组卷积重构的门控以更小开销换来更高精度。ImageNet 预训练权重的迁移效果如下表所示。

![ImageNet 预训练迁移效果](../assets/paper-imgs/EMCAD/table9.png)

预训练给 B0 带来 77.47% 到 81.97% 的大幅提升（mIoU 66.72%→72.64%），B2 也从 80.18% 升到 83.63%，除胆囊外全部器官都受益，验证了 ImageNet 预训练对编码器迁移的有效性。多阶段分割头的深监督（DS）消融结果如下表所示。

![深监督消融结果](../assets/paper-imgs/EMCAD/table10.png)

七个数据集中带深监督的 DICE 在六个上更高，Synapse 提升最大（82.03%→83.63%），仅 EM 略降（95.74%→95.53%），可见深监督对多数任务都有益。输入分辨率对 Synapse 精度与计算量的影响如下表所示。

![输入分辨率对 Synapse 精度与计算量的影响](../assets/paper-imgs/EMCAD/table11.png)

DICE 随分辨率单调上升：B0 从 224×224 的 81.97% 升到 512×512 的 85.52%（3.36G FLOPs），B2 升到 86.53%（22.39G FLOPs）；而 B2 在 256×256 下以 5.60G FLOPs 只得 84.47%，还比 B0 在 512×512 下低 1.05%，可见 B0 更适合搭配大分辨率输入。
