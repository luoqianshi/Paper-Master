...
ResNet50 基线为 77.26% Top-1 与 93.63% Top-5，嵌入 EMA 后达到 80.69% 与 95.59%，即 3.43% 与 1.96% 的增益；在几乎相同的计算复杂度下，EMA 的 Top-1 比 CA 高 0.52%，且参数量（23.71M 对 23.85M）与 FLOPs（1.32G 对 1.36G）更低。换用 ResNet101 后，CA 的 Top-1 从 80.17% 降到 80.01%，而 EMA 以 42.96M 参数与 2.53G FLOPs 取得 80.86% 与 95.75%，优于 46.22M 参数、2.54G FLOPs 的 CA，表明不做通道降维的策略在更深网络上依然稳定受益。
...
3. 模块轻量且即插即用，在 ResNet 与 YOLOv5 上参数增量仅 0.01M 至 0.22M，却在分类与无人机航拍检测上稳定超过 CBAM、SA、ECA、CA，实验说服力强。
